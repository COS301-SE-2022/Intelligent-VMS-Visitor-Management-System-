const ReceptionistSignButton = ({ htmlFor, text, colour, onClick, key, signInTime }) => {
    return (
        ...
        >
            {text}
            {signInTime != null ? (
                <div className="inline-flex absolute -top-2 -right-2 justify-center border-none bg-secondary items-center w-6 h-6 text-xs font-bold text-white rounded-full">{(new Date()).getDate()-(new Date(signInTime.replace(/-/gi,"/")).getDate())}</div>
            ):(
                <div></div>
            )}
