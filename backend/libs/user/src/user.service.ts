    ...
    async createUser(
        ...
        return this.commandBus.execute(
            new CreateUserCommand(
                ...
                name,
                badges,
                -1 * typeCounts["sleepover"],
                -1 * typeCounts["theme"],
                -1 * typeCounts["invite"],
                -100 * typeCounts["curfew"],
                dateString,
                pinNumber,
                file,
            ),
...
*/
    async createUser(
        ...
        return this.commandBus.execute(
            new CreateUserCommand(
                ...
                badges,
                -1 *
                    (isNaN(typeCounts["sleepover"])
                        ? 0
                        : typeCounts["sleepover"]),
                -1 * (isNaN(typeCounts["theme"]) ? 0 : typeCounts["theme"]),
                -1 * (isNaN(typeCounts["invite"]) ? 0 : typeCounts["invite"]),
                -100 * (isNaN(typeCounts["curfew"]) ? 0 : typeCounts["curfew"]),
                dateString,
                pinNumber,
                file,
            ),
        ...
        //     name,
        //     badges,
        //     -1 * (typeCounts["sleepover"] || 0),
        //     -1 * (typeCounts["theme"] || 0),
        //     -1 * (typeCounts["invite"] || 0),
        //     -100 * (typeCounts["curfew"] || 0),
        //     dateString,
        //     pinNumber,
        //     file,
        // ),
    ...
    async getUnAuthorizedUsers(permission: number) {
        return this.queryBus.execute(
            new GetUnAuthUsersQuery(permission === 0 ? -1 : -2),
        );
    }
    ...
    async authorizeUserAccount(email: string) {
        const res = await this.commandBus.execute(
            new AuthorizeUserCommand(email),
        );
    ...
    async deauthorizeUserAccount(email: string) {
        const res = await this.commandBus.execute(
            new DeauthorizeUserAccountCommand(email),
        );
        return res.modifiedCount > 0;
    }
    ...
    async getUsersByType(permission: number) {
        const users = await this.queryBus.execute(
            new GetUsersByTypeQuery(permission),
        );
        return users;
    }
    ...
    async getNumSleepovers(email: string) {
        ...
        const max = await this.restrictionService.getMaxSleepovers();
        return (
            (await this.queryBus.execute(new GetNumSleepoversQuery(email))) +
            max
        );
    }
    ...
    async getNumInvites(email: string) {
        const max = await this.restrictionService.getNumInvitesPerResident();
        // const max = (await this.restrictionService.getNumInvitesPerResident()).value;
        return (
            (await this.queryBus.execute(new GetNumInvitesQuery(email))) + max
        );
    }

    async getCurfewTimeOfResident(email: string) {
        //const max = (await this.restrictionService.getCurfewTime()).value;
        const max = await this.restrictionService.getCurfewTime();
        const time =
            (await this.queryBus.execute(new GetCurfewTimeQuery(email))) + max;
        return time % 2400;
    }

    async evaluateUser(email: string) {
        const sleepovers =
            await this.visitorInviteService.getTotalNumberOfSleepoversThisMonthOfResident(
                email,
            );
        ...
        for await (const [i, badge] of allBadges.entries()) {
            if (parseInt(badges.charAt(i)) < badge.levels) {
                switch (badge.type) {
                    case "invite":
                        variable =
                            await this.visitorInviteService.getTotalNumberOfInvitesOfResident(
                                email,
                            );
                        break;
                    // case "concept":
                    ...
                    case "cancellation":
                        variable =
                            await this.visitorInviteService.getTotalNumberOfCancellationsOfResident(
                                email,
                            );
                        break;
                    case "sleepover":
                        variable =
                            await this.visitorInviteService.getTotalNumberOfSleepoversOfResident(
                                email,
                            );
                        break;
                    case "time":
                    ...
                    case "visits":
                        variable =
                            await this.visitorInviteService.getTotalNumberOfVisitsOfResident(
                                email,
                            );
                        break;
                    case "suggestion":
